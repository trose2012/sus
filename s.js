// teacher: you know this is not funny right? you're not gonna get any marks for this
// me: i know, but i had fun
...
window.onload = async () => {
  ...
  try {
    ...
    const videoData = await fetch("s.mp4").catch(error);
    ...
    video.oncanplaythrough = async () => {
      ...
      push("Request URL", "/");
      push("Request Path", "/");
      push("Request Protocol", "HTTP/1.1"); // no fact checking
      ...
      push("Hacked By", "a gay furry"); // :trol:
      push("Current Status", "\u{D9E}"); // sus
    };
    ...
    start.onclick = async () => {
      ...
      const interval = setInterval(() => {
        ...
        // all it takes to make a different meme is to change the offset, bpm. thats it
